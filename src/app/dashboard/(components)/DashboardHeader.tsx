...
import { UserInfo, UserSignUp, getUserInfo } from '@/api/userApi';
...
export default function DashboardHeader({ children }: { children: React.ReactNode }) {
  ...
  return (
    <nav className={`absolute flex flex-row justify-end lg:justify-between w-[calc(100dvw-5rem)] items-center h-24 pr-20 z-10 bg-gray-300/50 `}>
      ...
      <div className="flex flex-row items-center w-fit h-full">
        ...
        <div className="flex flex-row justify-end items-center cursor-pointer" onClick={() => router.push('/mypage')}>
          ...
          <strong className="hidden md:flex md:mr-5">{myProfile.nickname}</strong>
        </div>
      </div>
    </nav>
  );
