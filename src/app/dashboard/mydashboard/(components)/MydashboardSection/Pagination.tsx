...
export default function Pagination({ count, page, setPage }: PaginationProps) {
  ...
  const disabledPrev = page === 1 ? 'pointer-events-none opacity-20' : '';
  const disabledNext = page === maxPage ? 'pointer-events-none opacity-20' : '';
    ...
    <div className="flex items-center">
      ...
      <div className="flex">
        <div className="flex flex-row rounded-md gap-[1px] border border-gary-D9D9D9 bg-gray-D9D9D9 overflow-hidden">
          ...
          >
            <Image className={`${disabledPrev}`} src="/images/arrow-forward-left.svg" alt="전 페이지 이동" width={18} height={18} />
          </button>
        </div>
        <div className="flex flex-row rounded-md gap-[1px] border border-gary-D9D9D9 bg-gray-D9D9D9 overflow-hidden">
          <button
          ...
          >
            <Image className={`${disabledNext}`} src="/images/arrow-forward-right.svg" alt="다음 페이지 이동" width={18} height={18} />
          </button>
        </div>
      </div>
    </div>
